...
if (isset($_SESSION['admin_name'])) {
    echo "<script>location.href='localhost/blog_single_php-main/admin/login.php'</script>";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($error)) {
        ...
        $isset = $db->fetchOne("admin", "email = '{$email}' AND password = '{$password}'");
        if ($isset > 0) {
            $_SESSION['admin_name'] = $isset['name'];
            $_SESSION['admin_id'] = $isset['id'];
            $_SESSION['admin_lv'] = intval($isset['level']);
            echo "<script>location.href='localhost/blog_single_php-main/admin/modules/index.php'</script>";
        } else {
