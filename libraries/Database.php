...
class Database{
    /**
     ...
     {
        $this->link = mysqli_connect("localhost","root","","sql") or die ();
...
/**
    ...
    mysqli_query($this->link, $sql) or die( "Lỗi truy vấn Update -- " .mysqli_error($this->link));
        ...
        public function fetchJones($sql, $page = 1, $total = 1, $row, $pagi = true)
        {
            $data = [];

            if ($pagi == true) {
                $sotrang = ceil($total / $row);
                $start = max(0, (intval($page) - 1) * $row);
                if ($start < 0) {
                    $start = 0;
                }
                $sql = "SELECT * FROM $sql LIMIT $start,$row";
                $data = ["page" => $sotrang];
        ...
        public function fetchJone($table, $sql, $row, $page = 1, $pagi = false)
        {
            $data = [];
